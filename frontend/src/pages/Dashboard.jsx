...
function Dashboard() {
  ...
  return (
    <>
      <nav className="bg-f7f6f2 h-20 sticky top-0 w-full z-50 flex justify-between items-center px-5 shadow-lg md:px-10">
        <div className="flex items-center gap-2 font-2xl">
          ...
          <h5 className="flex text-4xl font-bold">
            ...
            <span className="text-moneyantra-secondary">ANTRA</span>
          </h5>
        </div>

        <div className="hidden md:flex space-x-4">
          <a href="#" className="font-bold m-2 h-auto w-auto rounded-full text-base p-2 bg-[#33658a] text-white no-underline hover:underline">Dashboard</a>
        ...
        <div className="md:hidden">
          <button onClick={() => setIsOpen(!isOpen)} className="text-black focus:outline-none">
            {isOpen ? <IoMdClose className='text-4xl' /> : <RiMenu3Fill className='text-3xl' />}
          </button>
        </div>
      </nav>
      ...
      <div
        className={`
          fixed top-0 left-0 w-screen h-screen bg-gray-900 bg-opacity-95 z-40 md:hidden
